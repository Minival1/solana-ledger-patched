...
// Re-export so external callers can access via `solana_ledger::shred::merkle::SIZE_OF_MERKLE_PROOF_ENTRY`.
pub use crate::shred::merkle_tree::SIZE_OF_MERKLE_PROOF_ENTRY;
...
impl ShredData {
    ...
    // Exposed for external shred decoders (mirrors the accessors on the
    // shred_data::ShredData wrapper; v2.3.1 upstream only defined these on
    // the wrapper, not on merkle::ShredData, but jito's v2.2 merkle-recovery
    // branch did — callers that consume `merkle::Shred` directly rely on it).
    #[inline]
    pub fn last_in_slot(&self) -> bool {
        self.data_header.flags.contains(ShredFlags::LAST_SHRED_IN_SLOT)
    }
